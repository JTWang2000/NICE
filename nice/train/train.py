...
from nice.train.data_arguments import DataArguments, get_data_statistics
from nice.train.model_arguments import ModelArguments, add_padding_to_tokenizer
from nice.train.training_arguments import TrainingArguments
...
def main():
    ...
    # set all seed
    data_args.sample_data_seed = training_args.data_seed
    training_args.seed = training_args.data_seed
    ...
    elif "8B" in model_args.model_name_or_path:
        func_name = "encode_with_messages_format_with_llama3_chat"
    else:
        func_name = "encode_with_messages_format"
    print(func_name)
    # introducing external <pad> tokens can make fsdp behaves abnormally
    ...
    if training_args.analysis_mode:
        from nice.data_selection.get_validation_dataset import get_dataset
    ...
    if isinstance(model, PeftModel):
        print("remove full model parameters")
