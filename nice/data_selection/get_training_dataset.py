...
def get_training_dataset(train_files: List[str], tokenizer, max_seq_length, sample_percentage=1.0, seed=0,
    ...
    lm_datasets = encode_data(
        raw_datasets, tokenizer, max_seq_length, func_name=func_name)
    return lm_datasets
...
def encode_data(raw_datasets, tokenizer, max_seq_length, processing_num_workers=10, overwrite_cache=True,
                func_name="encode_with_messages_format"):
...
def get_encode_function(raw_datasets, tokenizer, max_seq_length, func="encode_with_messages_format"):
    ...
    if "prompt" in raw_datasets.column_names and "completion" in raw_datasets.column_names:
        ...
        elif func == "encode_with_final_messages_format":
            encode_func = encode_with_final_messages_format
        elif func == "encode_with_messages_format_with_llama2_chat":
            encode_func = encode_with_messages_format_with_llama2_chat
        elif func == "encode_with_final_messages_format_with_llama2_chat":
            encode_func = encode_with_final_messages_format_with_llama2_chat
        elif func == "encode_with_messages_format_with_llama3_chat":
            encode_func = encode_with_messages_format_with_llama3_chat
        else:
            encode_func = encode_with_final_messages_format_with_llama3_chat
        encode_function = partial(
            encode_func,
            tokenizer=tokenizer,
            max_seq_length=max_seq_length,
...
def encode_with_messages_format(example, tokenizer, max_seq_length):
    ...
    if example['dataset'] == 'hh_rlhf':
        return encode_with_final_messages_format(example, tokenizer, max_seq_length)
...
def encode_with_final_messages_format(example, tokenizer, max_seq_length):
    '''
    Original implementation of the function: https://github.com/allenai/open-instruct/blob/9ebcb582cfc243a6dab75b4302fa432784db26c2/open_instruct/finetune.py#L264C1-L322C1
    ...
    # only save the final assistant sentence to compute loss
    final_message = messages[-1]
    message_idx = len(messages) - 1
    assert final_message["role"] == "assistant" and message_idx != 0
    message_start_idx = tokenizer(
        concat_messages(messages[:message_idx], tokenizer) + "<|assistant|>\n", return_tensors='pt',
        max_length=max_seq_length, truncation=True).input_ids.shape[1]
    labels[:, message_start_idx:] = input_ids[:, message_start_idx:]
...
def encode_with_messages_format_with_llama2_chat(example, tokenizer, max_seq_length):
    ...
    if len(messages) == 0:
        raise ValueError('messages field is empty.')
    if example['dataset'] == 'hh_rlhf':
        return encode_with_final_messages_format_with_llama2_chat(example, tokenizer, max_seq_length)
...
def encode_with_final_messages_format_with_llama2_chat(example, tokenizer, max_seq_length):
    '''
    Here we assume each example has a 'messages' field Each message is a dict with 'role' and 'content' fields.
    We concatenate all messages with the roles as delimiters and tokenize them together.
    "<s> [INST] Hello, how are you? [/INST]I'm doing great. How can I help you today?</s> [INST] I'd like to show off how chat templating works! [/INST]"
...
def encode_with_messages_format_with_llama3_chat(example, tokenizer, max_seq_length):
    '''
    Here we assume each example has a 'messages' field Each message is a dict with 'role' and 'content' fields.
    We concatenate all messages with the roles as delimiters and tokenize them together.
    Adapted from https://github.com/meta-llama/llama3/blob/main/llama/tokenizer.py#L206
    ...
    if len(messages) == 0:
        raise ValueError('messages field is empty.')
    if example['dataset'] == 'hh_rlhf':
        return encode_with_final_messages_format_with_llama3_chat(example, tokenizer, max_seq_length)
...
def encode_with_final_messages_format_with_llama3_chat(example, tokenizer, max_seq_length):
    '''
    Here we assume each example has a 'messages' field Each message is a dict with 'role' and 'content' fields.
    We concatenate all messages with the roles as delimiters and tokenize them together.
    Adapted from https://github.com/meta-llama/llama3/blob/main/llama/tokenizer.py#L206
    ...
    assert final_message["role"] == "assistant" and message_idx != 0
    message_start_idx = tokenizer(
        _concat_messages(messages[:message_idx], tokenizer) + f"{B_HEADER}assistant{E_HEADER}\n\n",
